...
def add_comment(comment_text):
    """Add a comment to the MLIR at the current point"""
    
    # Firstly, replace all @ in comment_text with []
    comment_text = re.sub(r'@', '[]', comment_text)
    
    (frame, filename, line_number, function_name, lines, index) = getProperFrame()
    lt.addComment(ctxt, comment_text.encode('utf-8'), filename.encode('utf-8'), line_number)

# Export this function
lt.addComment.argtypes = [ctypes.c_void_p, ctypes.c_char_p, ctypes.c_char_p, ctypes.c_size_t]
lt.addComment.restype = None