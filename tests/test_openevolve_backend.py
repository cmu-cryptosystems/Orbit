...
def test_transition_score_keeps_reserve_regularization_as_search_heuristic(toy_cost_json: str):
    params = _params(toy_cost_json)
    base_policy = {
    ...
    assert ignored_score == 0.0
    assert 0.0 < avgcase_score < worst_score
...
def test_initial_compile_seed_exposes_active_bootstrap_mcts_knobs(
    ...
    context["harness"]["enable_trace_patch_initial"] = True
    program_path = tmp_path / "initial.py"
...
def test_promotion_probe_hints_are_lightweight_and_targeted(toy_cost_json: str):
    hints = {
        "strategy": "bootstrap_mcts",
        "mcts_action_presets": {
            "budget_fulfillment_beam": {
        ...
        "boundary_group_policies": [
            ...
            {
                "selector": {"in_lvl": 8, "in_scl": 51, "maino_v": "", "main_dag_size": 0},
                "policy": {"boundary_state_cap": 8, "max_scale_candidates": 48},
            },
        ],
        "unit_policies": [
            {"selector": {"layer": "0"}, "policy": {"boundary_state_cap": 8}},
        ],
    }
    tasks = [
        {
            "group_keys": [
                {"in_lvl": -1, "in_scl": 40, "maino_v": "", "main_dag_size": 0}
            ],
...
def test_experience_probe_uses_top_groups_before_full_sampled_replay(
    ...
    monkeypatch.setenv("ORBIT_OPENEVOLVE_INLINE_QBP", "1")
...
def test_default_experience_surrogate_does_not_run_inline_qbp(
    toy_cost_json: str, tmp_path: Path, monkeypatch
):
    monkeypatch.delenv("ORBIT_OPENEVOLVE_INLINE_QBP", raising=False)
    monkeypatch.setenv("ORBIT_OPENEVOLVE_EXPERIENCE_PROBE_EVERY", "1")
    ...
    def fail_sampled(*_args, **_kwargs):
        raise AssertionError("inline QBP should be disabled by default")
    ...
    result = oe_backend._experience_probe_skip_result(
        context,
        {
            "strategy": "bootstrap_mcts",
            "boundary_state_cap": 8,
            "mcts_actions": [{"name": "wide_boundary_cost_beam"}],
        },
        {
            "strategy": "bootstrap_mcts",
            "boundary_state_cap": 8,
    ...
    assert result is not None
    assert result["artifacts"]["failure_stage"] == "experience_surrogate_feature_only"
    assert result["metrics"]["candidate_stage_rank"] == 0.0
    assert result["metrics"]["experience_probe_promoted"] == 0.0
...
def test_sampled_only_dp_probe_selects_without_materialization(
    toy_cost_json: str, tmp_path: Path, monkeypatch
):
    params = _params(
        toy_cost_json,
    ...
    def fake_probe(_context, _hints, _params, _tasks, *, timeout_sec):
        assert timeout_sec >= 0
        return {
            "valid": True,
            "boundary_group_validity": 1.0,
            ...
            "bootstrap_count": 2,
            "rescale_count": 4,
            "sampled_selected_path_digest": "candidate-path",
            "diagnostics": {
                "fallback_selected_boundary_groups": 0,
                "invalid_boundary_groups": 0,
    ...
    def fail_materialization(*_args, **_kwargs):
        raise AssertionError("sampled-only DP table probe should not materialize")

    monkeypatch.setattr(oe_backend, "_evaluate_promotion_probe", fake_probe)
    monkeypatch.setattr(oe_backend, "_evaluate_dp_probe_for_promotion", fail_materialization)
    monkeypatch.setattr(oe_backend, "_evaluate_sampled_budget_tasks_for_promotion", fail_materialization)
    ...
    assert selected is not None
    assert selected["marker"] == "fast"
    assert selected["openevolve_promotion_stage"] == "dp_table_probe_sampled_only"
    assert selected["openevolve_promotion_requires_full_materialization"] is True
    assert selected["openevolve_promotion_latency_usec"] == 80.0
    assert selected["openevolve_promotion_reference_latency_usec"] == 100.0
    summary = json.loads((tmp_path / "sampled_promotion" / "promotion_summary.json").read_text())
    assert summary["selected"] is True
    assert summary["selected_record"]["stage"] == "dp_table_probe_sampled_only"
...
def test_strategy_discovery_micro_probe_keeps_one_output_per_group(
    toy_cost_json: str,
):
    params = _params(toy_cost_json, openevolve_eval_suite="polybert-sampled")
    graph = Tdag(params, "micro_probe_graph")
    graph.add_node("arg0", op="input", weight=1, op_descr={}, comment="")
    graph.add_node("out", op="add", weight=1, op_descr={}, comment="")
    graph.add_edge("arg0", "out")
    graph.inputs = {"arg0"}
    graph.outputs = {"out"}
    task = {
        "index": 5,
        ...
        "group_keys": [
            {"in_lvl": 8, "in_scl": 120, "maino_v": "", "main_dag_size": 0},
            {"in_lvl": 16, "in_scl": 51, "maino_v": "", "main_dag_size": 0},
        ],
        "context": build_context(
            graph,
            [
                {"in_lvl": 8, "in_scl": 120, "out_lvl": 1},
                {"in_lvl": 8, "in_scl": 120, "out_lvl": 8},
                {"in_lvl": 8, "in_scl": 120, "out_lvl": 16},
                {"in_lvl": 16, "in_scl": 51, "out_lvl": 1},
                {"in_lvl": 16, "in_scl": 51, "out_lvl": 4},
                {"in_lvl": 16, "in_scl": 51, "out_lvl": 16},
            ],
            params,
        ),
    }

    micro_tasks = oe_backend._strategy_discovery_micro_probe_tasks([task])

    assert len(micro_tasks) == 1
    budgets = micro_tasks[0]["context"]["io_budgets"]
    assert len(budgets) == 2
    assert {(item["in_lvl"], item["in_scl"], item["out_lvl"]) for item in budgets} == {
        (8, 120, 8),
        (16, 51, 16),
    }
    assert micro_tasks[0]["strategy_micro_budget_count"] == 2
    assert micro_tasks[0]["group_keys"] == task["group_keys"]


def test_strategy_discovery_micro_probe_skips_full_seed_equivalent(
    toy_cost_json: str, tmp_path: Path, monkeypatch
):
    monkeypatch.setenv("ORBIT_OPENEVOLVE_STRATEGY_DISCOVERY_VARIANTS", "1")
    params = _params(
        toy_cost_json,
        openevolve_eval_suite="polybert-sampled",
        openevolve_search_mode="bootstrap-mcts",
    ...
    monkeypatch.setattr(
        oe_backend,
        "_strategy_discovery_variants",
        lambda *_args, **_kwargs: [
            {
                "label": "same_micro",
                "dimension": "boundary_scale_policy",
                "changed_keys": ["boundary_scale_policy"],
                "hints": {
                    "strategy": "bootstrap_mcts",
                    "qbp_engine": "dp",
                    "boundary_scale_policy": "frontier",
    ...
    def fake_dp_probe(_context, _hints, selected_tasks, **kwargs):
        calls.append(
            {
                "budget_count": sum(
                    len(task["context"]["io_budgets"]) for task in selected_tasks
                ),
                "materialize": bool(kwargs.get("materialize", False)),
            }
        )
        return {
            "valid": True,
            ...
            "bootstrap_count": 1.0,
            "rescale_count": 2.0,
            "sampled_selected_path_digest": "seed-micro",
            "diagnostics": {
                "fallback_selected_boundary_groups": 0,
                "invalid_boundary_groups": 0,
                "candidate_solved_boundary_groups": 1,
    ...
    discovery = oe_backend._run_strategy_discovery_prepass(
        tmp_path,
        context,
        {"strategy": "bootstrap_mcts", "qbp_engine": "dp"},
        params,
    ...
    assert len(calls) == 2
    assert all(item["budget_count"] == 1 for item in calls)
    assert calls[0]["materialize"] is False
    assert calls[1]["materialize"] is False
    assert discovery["records"][0]["reason"] == "micro_probe_seed_equivalent"
    assert discovery["summary"]["latency_improved_count"] == 0


def test_strategy_discovery_micro_probe_keeps_slower_path_as_example(
    toy_cost_json: str, tmp_path: Path, monkeypatch
):
    monkeypatch.setenv("ORBIT_OPENEVOLVE_STRATEGY_DISCOVERY_VARIANTS", "1")
    params = _params(
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: [
            {
                "label": "slower_path",
                "dimension": "dp_probe_mode",
                "changed_keys": ["dp_probe_mode"],
                "hints": {
                    "strategy": "bootstrap_mcts",
                    "qbp_engine": "dp",
    ...
    def fake_dp_probe(_context, _hints, selected_tasks, **kwargs):
        ...
        idx = len(calls)
        digest = "seed-micro" if idx == 1 else "slower-micro"
        latency = 100.0 if idx == 1 else 150.0
        return {
            "valid": True,
            "boundary_group_validity": 1.0,
            "candidate_qbp_coverage": 1.0,
    ...
    assert len(calls) == 2
    assert all(item["budget_count"] == 1 for item in calls)
    assert discovery["records"][0]["reason"] == "micro_probe_changed_slower"
...
def test_sampled_promotion_clamps_before_complexity_reject(
    toy_cost_json: str, tmp_path: Path, monkeypatch
):
    params = _params(toy_cost_json, openevolve_eval_suite="polybert-sampled")
    context = build_compile_context(_mul_chain_pdag(params, length=3), params)
    ...
    def fake_eval(_context, hints, *, timeout_sec):
        evaluated.append(hints)
        assert hints["mcts_action_presets"]["dense_boundary_cost_beam"]["policy"][
            "max_scale_candidates"
        ] == 48
        return {
            "valid": True,
    ...
    monkeypatch.setattr(
        ...
        lambda context, initial_hints, params, selected, reference, timeout_sec=0: (
            ...
            100.0,
            {
                ...
                "bootstrap_count": 0.0,
                "rescale_count": 0.0,
                "diagnostics": {"selected_source_counts": {"seed:qbp_dp": 1}},
            },
        ),
    )
    calls = []
...
def test_dp_promotion_rejects_slower_changed_probe_without_materialization(
    toy_cost_json: str, tmp_path: Path, monkeypatch
):
    params = _params(
        toy_cost_json,
    ...
    def fake_dp_probe(*_args, **kwargs):
        ...
        if kwargs.get("materialize", False):
            raise AssertionError("slower DP probe should not materialize")
        return {
            "valid": True,
            "boundary_group_validity": 1.0,
            "candidate_qbp_coverage": 1.0,
            ...
            "fallback_selected_groups": 0,
            "invalid_boundary_groups": 0,
            "sampled_dp_latency_usec": 110.0,
            "objective_cost_usec": 110.0,
            "bootstrap_count": 0.0,
            "rescale_count": 0.0,
            "sampled_selected_path_digest": "changed-slower",
            "diagnostics": {
                "fallback_selected_boundary_groups": 0,
                "invalid_boundary_groups": 0,
                "candidate_solved_boundary_groups": 1,
                ...
                "sampled_task_metrics": [
                    {
                        "selected_path_digest": "changed-slower",
                        "sampled_dp_latency_usec": 110.0,
                        "bootstrap_count": 0.0,
                        "rescale_count": 0.0,
                        "boundary_group_count": 1,
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("slower DP probe should not run sampled replay")
        ),
    )
    ...
    summary = json.loads((tmp_path / "sampled_promotion" / "promotion_summary.json").read_text())
    assert summary["records"][0]["reason"] == "dp_changed_slower"
