...
for model in ResNet; do
    for act in ReLU SiLU; do
        ...
        python scripts/optimizer/saturn/run_saturn.py --model $model --act $act --n 64 --Lm 16 --Sw 40 --nobypass
...
for model in ResNet; do
    for act in SiLU ReLU; do
        ...
        python scripts/optimizer/saturn/run_saturn.py --model $model --act $act --n 16 --Lm 16 --Sw 40 --nobypass
        echo "  Finished Saturn optimization for model: $model, activation: $act"
    done
done
