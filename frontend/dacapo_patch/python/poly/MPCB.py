...
def shapeClosure(nt, bb, fh, fw, s , hi, ho, wi, wo, ni, no, ci, co, ki, ko, ti, to, pi, po, q) :
    ...
    def MultParPack (name, A) :
        hc.add_comment(f"// multparpack@{name}")
        A = A/bb
    ...
    def Downsamp (name, A) :
        ...
        hc.add_comment(f"// downsamp@{name}")
        C = np.full( (ni) ,Empty(), dtype = object)
        S = DownSelecting()
...
#                 i8 = (ki*i2 + i1) // cint(co // no // ki)
    ...
    def AvgMidPool (name, A) :
        hc.add_comment(f"// avgmidpool@{name}")
        #B = torch.zeros((no, nt), dtype=torch.double)
...
#             reduces = [M] + reduces[:-2]
#         A = reduces[0]
        hc.add_comment(f"// maxpool@{name}")
        B = np.full((ni), Empty(), dtype=object)
        C = np.full((no), Empty(), dtype=object)
        for ii in range ( ni ) : 
            B[ii] = A[ii]
            for j in range (fint (np.log2(fw))) : 
                hc.add_comment(f"// rol@{name}_{ii}_fw_{j}_rol")
                tmp = roll (B[ii], - (pow(2, j) * ki))
                B[ii] = maximum(f"{name}_{ii}_fw_{j}_maxx", B[ii], tmp)
            for j in range (fint (np.log2(fh))) : 
                hc.add_comment(f"// rol@{name}_{ii}_fh_{j}_rol")
                tmp = roll (B[ii], - (pow(2, j) * ki * ki * wi))
                B[ii] = maximum(f"{name}_{ii}_fh_{j}_maxx", B[ii], tmp)

        hc.add_comment(f"// mul@{name}_final")
...
#             reduces = [M] + reduces[:-2]
#         A = reduces[0]
        hc.add_comment(f"// maxpoolpad@{name}")
        B = np.full((ni), Empty(), dtype=object)
        C = np.full((no), Empty(), dtype=object)
        M = ParMPDM()
        D = ParMPDA()
...
#                 B[ii] = maximum(B[ii], roll (B[ii], - (j - (fh // 2) * ki * ki * wi))* M[j, 0] + D[j, 0])
        for ii in range ( ni ) : 
            B[ii] = A[ii]
            for j in range (fw) : 
                hc.add_comment(f"// rolmul@{name}_{ii}_fw_{j}_mulrol")
                tmp = roll (B[ii], - (j - (fw-1) // 2)* ki)  * M[1, j ] + D[1,j]
                B[ii] = maximum(f"{name}_{ii}_fw_{j}_maxx", B[ii], tmp)
            for j in range (fh) : 
                hc.add_comment(f"// rolmul@{name}_{ii}_fh_{j}_mulrol")
                tmp = roll (B[ii], - (j - (fh-1) // 2) * ki * ki * wi) * M[j, 1] + D[j,1]
                B[ii] = maximum(f"{name}_{ii}_fh_{j}_maxx", B[ii], tmp)
        hc.add_comment(f"// mulrol@{name}_final")
        S = DownSelecting()
        for i1 in range (ki) :
            for i2 in range (ti) :
                i3 = ((ki*i2 + i1)%(2*ko))//2
...
#             C[0] = C[0] + roll (C[0], pow(2, j) * (nt // po))
    ...
    def MultParBN(name, A, G, H) : 
        hc.add_comment(f"// bn@{name}")
        D = np.full( (no) ,Empty(), dtype = object)
        for ii in range(no) :
    ...
    def MultParConv (name, A, U, bias) :
        hc.add_comment(f"// conv@{name}")
        # D = torch.zeros ( (nt) , dtype=torch.double)
        #D = torch.zeros ( (no, nt) , dtype=torch.double)
        D = np.full( (no) ,Empty(), dtype = object)
        # tmp = torch.zeros ( (fh, fw, nt), dtype=torch.double)
    ...
    def MultParConvBN (name, A, U, G, H) :
        hc.add_comment(f"// convbn@{name}")
        # D = torch.zeros ( (nt) , dtype=torch.double)
        #D = torch.zeros ( (no, nt) , dtype=torch.double)
        D = np.full( (no) ,Empty(), dtype = object)
        # tmp = torch.zeros ( (fh, fw, nt), dtype=torch.double)
...
def Linear (name, A, U, B, nt) :
    hc.add_comment(f"// linear@{name}")
...
def BN (name, A, G, H, nt):
    hc.add_comment(f"// mpbn@{name}")
    R = np.full( (1) ,Empty(), dtype = object)
    #R = torch.zeros((1, nt), dtype = torch.double)
