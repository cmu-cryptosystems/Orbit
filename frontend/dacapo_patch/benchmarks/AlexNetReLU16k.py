...
def getModel():
    ...
    # model_dict = torch.load(str(source_dir)+"/../data/alexNet_silu_maxpool_model", map_location=torch.device('cpu'))
...
def AlexNetReLU16k (ctxt) :
    ...
    out = HE_ConvBN("convbn1", close, input_var,model.module.Conv2d_1, model.module.bn_1)
    ...
    out = pooling("maxpool1", close, out)
    block_in = maxpool_1_shapes
    
    print("Conv_2_BN")        
    conv_2_shapes = CascadeConv(block_in, model.module.Conv2d_2)
    close = shapeClosure(**conv_2_shapes)
    out = HE_ConvBN("convbn2", close, out,model.module.Conv2d_2, model.module.bn_2)
    # out = hc.bootstrap(out)
    out = act("act2", out)
    block_in = conv_2_shapes
 
    print("maxpool_2")
    maxpool_2_shapes = CascadeMax (block_in, model.module.maxpool_2)
    close = shapeClosure(**maxpool_2_shapes)
    out = pooling("maxpool2", close, out)
    # out = hc.bootstrap(out)
    block_in = maxpool_2_shapes

    print("Conv_3")        
    conv_3_shapes = CascadeConv(block_in, model.module.Conv2d_3)
    close = shapeClosure(**conv_3_shapes)
    out = HE_ConvBN("convbn3", close, out ,model.module.Conv2d_3, model.module.bn_3)
    # out = hc.bootstrap(out)
    out = act("act3", out)
    ...
    out = HE_ConvBN("convbn4", close, out ,model.module.Conv2d_4, model.module.bn_4)
    # out = hc.bootstrap(out)
    out = act("act4", out)
    ...
    out = HE_ConvBN("convbn5", close, out ,model.module.Conv2d_5, model.module.bn_5)

    # out = hc.bootstrap(out)
    out = act("act5", out)
    block_in = conv_5_shapes
    print("maxpool_3")
    maxpool_3_shapes = CascadeMax (block_in, model.module.maxpool_3)
    close = shapeClosure(**maxpool_3_shapes)
    out = pooling("maxpool3", close, out)
    # out = hc.bootstrap(out)
    block_in = maxpool_3_shapes
    
    print("fc_1")
    out = HE_ReshapeLinear("fc_1", close["OP"], out, model.module.fc_1, scale = 32.0, reshape = block_in)
    # out = hc.bootstrap(out)
    out = act("act6", out)
    print("fc_2")
    out = HE_Linear("fc_2", close["OP"], out, model.module.fc_2, scale = 32.0)
    # out = hc.bootstrap(out)
    out = act("act7", out)
    print("fc_3")
