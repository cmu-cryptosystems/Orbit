...
echo "Start Default Running all 32k ReSBM"
for model in "${model_32[@]}"; do
    for act in "${acts[@]}"; do
        echo "    Running model: $model, act: $act"
        python -u -m scripts.optimizer.ReSBM.optimizer --inputfile mlirs_input/${model}${act}32k.mlir --outputfile mlirs_output/ReSBM_1_14_${model}${act}32k.mlir --costjson cost_models/lattigo_nepnew_64k_1_14_sim_vari.json > mlirs_output/ReSBM_1_14_${model}${act}32k.log 2>&1
        echo "        Finished running model: $model, act: $act"
    done
done
echo "Finished Default Running all 32k ReSBM"
