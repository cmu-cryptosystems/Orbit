...
def getModel():
    ...
    model_dict = torch.load(str(source_dir)+"/../data/resnet20_relu_model", map_location=torch.device('cpu'))
    new_model_dict = {}
    for k, v in model_dict.items():
        new_model_dict[f"module.{k}"] = v
...
def ResNetReLU16k (ctxt) :
    ...
    out = HE_ConvBN("convbn1", close, input_var,model.module.conv1, model.module.bn1)
    ...
    for i in range(0, len(model.module.layer1)) :
        ...
        out = HE_ConvBN(f"layer1_{i}_convbn1", close, out, model.module.layer1[i].conv1, model.module.layer1[i].bn1)
        # out = hc.bootstrap(out)
        out = act(f"layer1_{i}_act1", out)
    ...
    dsout = HE_DS("layer3_ds", close, out)
    for i in range(0, len(model.module.layer3)) :
        ...
        close = shapeClosure(**inconv1_shapes)
        out = HE_ConvBN(f"layer3_{i}_convbn1", close, out, model.module.layer3[i].conv1, model.module.layer3[i].bn1)
        # out = hc.bootstrap(out)
        out = act (f"layer3_{i}_act1", out)
        inconv2_shapes = CascadeConv (inconv1_shapes, model.module.layer3[i].conv2)
        close = shapeClosure(**inconv2_shapes)
        out = HE_ConvBN(f"layer3_{i}_convbn2", close, out, model.module.layer3[i].conv2, model.module.layer3[i].bn2)
        hc.add_comment(f"// add@layer3_{i}_add")
        out= out + dsout
        # out = hc.bootstrap(out)
        out = act (f"layer3_{i}_act2", out)
        block_in = inconv2_shapes
