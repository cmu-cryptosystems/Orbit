...
for model in ResNet; do
    for act in ReLU SiLU; do
        ...
        python3 scripts/optimizer/orbit/run_orbit.py --threads "${ORBIT_THREADS:-32}" --model $model --act $act --n 64 --Lm 16 --Sw 40 --nobypass
...
for model in ResNet; do
    for act in SiLU ReLU; do
        ...
        python3 scripts/optimizer/orbit/run_orbit.py --threads "${ORBIT_THREADS:-32}" --model $model --act $act --n 16 --Lm 16 --Sw 40 --nobypass
        echo "  Finished Orbit optimization for model: $model, activation: $act"
    done
done