...
def getModel():
    ...
    # model_dict = torch.load(str(source_dir)+"/../data/vgg16_silu_maxpool_model", map_location=torch.device('cpu'))
...
def VGG16ReLU16k (ctxt) :
    ...
    out = pooling("maxpool_2", close, out)
    # out = hc.bootstrap(out)
    ...
    out = pooling("maxpool_3", close, out)
    # out = hc.bootstrap(out)
    ...
    print("maxpool_5")
    maxpool_5_shapes = CascadeMax (block_in, model.module.maxpool_5)
    close = shapeClosure(**maxpool_5_shapes)
    out = pooling("maxpool_5", close, out)
    # out = hc.bootstrap(out)
    block_in = maxpool_5_shapes
